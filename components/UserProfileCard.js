    ...
    <div className="bg-white/95 backdrop-blur-sm rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border border-white/20 overflow-hidden">
      ...
      <div className="p-4 sm:p-6 border-b border-gray-100">
        <div className="flex items-start space-x-3 sm:space-x-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm sm:text-base text-gray-600 mb-2">@{user.login}</p>
            {user.bio && (
              <p className="text-xs sm:text-sm text-gray-700 mb-3">{user.bio}</p>
            )}
            <div className="flex flex-wrap gap-2">
              ...
              {user.blog && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center space-x-1 text-green-600 hover:text-green-800 text-xs sm:text-sm"
                >
      ...
      <div className="px-4 sm:px-6 py-4 bg-gray-50/50">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="text-center">
            <div className="text-xl sm:text-2xl font-bold text-blue-600">{user.public_repos}</div>
            <div className="text-xs sm:text-sm text-gray-600">Repositories</div>
          </div>
          <div className="text-center">
            <div className="text-xl sm:text-2xl font-bold text-yellow-600">{formatNumber(stats.totalStars)}</div>
            <div className="text-xs sm:text-sm text-gray-600">Total Stars</div>
          </div>
          <div className="text-center">
            <div className="text-xl sm:text-2xl font-bold text-green-600">{formatNumber(stats.totalForks)}</div>
            <div className="text-xs sm:text-sm text-gray-600">Total Forks</div>
          </div>
          <div className="text-center">
            <div className="text-xl sm:text-2xl font-bold text-purple-600">{formatNumber(stats.contributions)}</div>
      ...
      {stats.topRepos && stats.topRepos.length > 0 && (
        <div className="px-4 sm:px-6 py-4 border-t border-gray-100">
          <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-3">Top Repositories</h3>
      ...
      {/* Footer */}
      <div className="px-4 sm:px-6 py-3 bg-gray-50/30 border-t border-gray-100">
